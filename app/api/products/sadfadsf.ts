...
export async function DELETE(req: NextRequest, { params }: { params: { categoryId: Array<string> } }) {
    try {
        ...
        if (!id) {
            return NextResponse.json("problem indentifiying the id", { status: 405 });
        } else {
            ...
            return NextResponse.json("Product Deleted Successfuly", { status: 202 });
        }
    } catch (e) {
        console.log("there was an error deleting a product", e);
