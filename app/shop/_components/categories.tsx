...
function CategoryEntry({ cate }: { cate: Category & { imageName: Array<{ imageName: string }> } }) {
  ...
  return (
    <div ref={ref} className={clsx({ "opacity-100 translate-x-[0px]": inView }, "hover:p-2 translate-x-[-50px]  transition-all duration-700 ease-in-out")}>
      <Link href={"/shop"}>
        <div className={`p-4 relative hover:w-[270px] transition-all duration-700 hover:h-[220px] rounded-xl flex flex-col bg-pr items-center w-[240px] h-[190px]`}>
  ...
  return (
    <div>
      <Element name={'categories'}>
        <div className="bg-td flex flex-col justify-center items-center gap-8 py-[40px] pb-[60px] ">
